...
#define AF_MAX_BUFFER_SIZE 1048576 // 1MB
...
/*** DOCUMENTATION
                ...
                <syntax>
                        <parameter name="wsserver" required="true" argsep=".">
                                <argument name="wsserver" required="true">
                                        <para>the URL to the  websocket server
 you want to send the audio to. </para>
                                </argument>
                        ...
                        <parameter name="options">
                                <optionlist>
                                        <option name="b">
                                                <para>Only save audio to the
 file while the channel is bridged.</para> <note><para>If you utilize this
 option inside a Local channel, you must make sure the Local channel is not
 optimized away. To do this, be sure to call your Local channel with the
                                                <literal>/n</literal> option.
 For example: Dial(Local/start@mycontext/n)</para></note>
                                        </option>
                                        <option name="B">
                                                <para>Play a periodic beep while
 this call is being recorded.</para> <argument name="interval"><para>Interval,
 in seconds. Default is 15.</para></argument>
                                        </option>
                                        <option name="v">
                                                <para>Adjust the
 <emphasis>heard</emphasis> volume by a factor of <replaceable>x</replaceable>
                                                (range <literal>-4</literal> to
 <literal>4</literal>)</para> <argument name="x" required="true" />
                                        </option>
                                        <option name="V">
                                                <para>Adjust the
 <emphasis>spoken</emphasis> volume by a factor of <replaceable>x</replaceable>
 (range <literal>-4</literal> to <literal>4</literal>)</para> <argument name="x"
 required="true" />
                                        </option>
                                        <option name="W">
                                                <para>Adjust both,
 <emphasis>heard and spoken</emphasis> volumes by a factor of
 <replaceable>x</replaceable> (range <literal>-4</literal> to
 <literal>4</literal>)</para> <argument name="x" required="true" />
                                        </option>
                                        <option name="r">
                                                <argument name="file"
 ...
 configured monitoring directory.</para>
                                        ...
                                        <option name="S">
                                                <para>When combined with the
 <replaceable>r</replaceable> or <replaceable>t</replaceable> option, inserts
 silence when necessary to maintain synchronization between the receive and
 transmit audio streams.</para>
                                        </option>
                                        <option name="i">
                                                <argument name="chanvar"
 required="true" /> <para>Stores the AudioFork's ID on this channel
 variable.</para>
                                        </option>
                                        <option name="p">
                                                <para>Play a beep on the channel
 that starts the recording.</para>
                                        </option>
                                        <option name="P">
                                                <para>Play a beep on the channel
 that stops the recording.</para>
                                        </option>
                                        <option name="D">
                                                <para>Direction of audiohook to
 process - supports in, out, and both</para>
                                        </option>
                                        <option name="T">
                                                <para>comma separated TLS config
 for secure websocket connections</para>
                                        </option>
                                        <option name="R">
                                                <para>Timeout for
 reconnections</para>
                                        ...
                                        <option name="r">
                                                <para>Number of times to attempt
 reconnect before closing connections</para>
                                        </option>
                                </optionlist>
                        </parameter>
                        <parameter name="command">
                                <para>This is executed when the audio fork's
 hook finishes</para> <para>Any strings matching <literal>^{X}</literal> will be
 unescaped to <variable>X</variable>.</para> <para>All variables will be
 evaluated at the time AudioFork is called.</para> <warning><para>Do not use
 untrusted strings such as <variable>CALLERID(num)</variable> or
 <variable>CALLERID(name)</variable> as part of the command parameters.  You
                                risk a command injection attack executing
 arbitrary commands if the untrusted strings aren't filtered to remove dangerous
 characters.  See function <variable>FILTER()</variable>.</para></warning>
                        </parameter>
                </syntax>
                <description>
                        <para>Forks raw audio to a remote websocket.</para>
                        <para>This application does not automatically answer and
 should be preceeded by an application such as Answer or Progress().</para>
                        <note><para>AudioFork runs as an
 audiohook.</para></note> <variablelist> <variable name="AUDIOFORK_WSSERVER">
                                        <para>The URL of the websocket
 server.</para>
                                </variable>
                        </variablelist>
                        <warning><para>Do not use untrusted strings such as
 <variable>CALLERID(num)</variable> or <variable>CALLERID(name)</variable> as
 part of ANY of the application's parameters.  You risk a command injection
 attack executing arbitrary commands if the untrusted strings aren't filtered to
 remove dangerous characters.  See function
 <variable>FILTER()</variable>.</para></warning>
        ...
        <application name="StopAudioFork" language="en_US">
                <synopsis>
                        Cancels an ongoing audio fork and closes the websocket
 connection.
                </synopsis>
                <syntax>
                        <parameter name="AudioForkID" required="false">
                                <para>If a valid ID is provided, then this
 command will stop only that specific AudioFork.</para>
                        </parameter>
                </syntax>
                <description>
                        <para>Stop an ongoing AudioFork created previously by
 <literal>AudioFork()</literal> on the current channel.</para>
                </description>
                <see-also>
                        <ref type="application">AudioFork</ref>
                </see-also>
 ...
 /> <parameter name="Channel" required="true"> <para>Used to specify the channel
 to mute.</para>
                        </parameter>
                        <parameter name="Direction">
                                <para>Which part of the audio fork to mute:
 read, write or both (from channel, to channel or both channels).</para>
                        </parameter>
                        <parameter name="State">
                                <para>Turn mute on or off : 1 to turn on, 0 to
 turn off.</para>
                        </parameter>
                </syntax>
                <description>
                        <para>This action may be used to mute a AudioFork
 session.</para>
                </description>
        </manager>
 ...
 to record.</para>
                        ...
                        <parameter name="WsServer">
                                <para>The websocket server URL to fork audio
 to.</para>
                        </parameter>
                        <parameter name="options">
                                <para>Options that apply to the AudioFork in the
 same way as they would apply if invoked from the AudioFork application. For a
 list of available options, see the documentation for the audiofork application.
 </para>
                        </parameter>
                        <parameter name="Command">
                                <para>Will be executed when the audio fork has
 completed. Any strings matching <literal>^{X}</literal> will be unescaped to
 <variable>X</variable>. All variables will be evaluated at the time AudioFork
 is called.</para> <warning><para>Do not use untrusted strings such as
 <variable>CALLERID(num)</variable> or <variable>CALLERID(name)</variable> as
 part of the command parameters.  You risk a command injection attack executing
 arbitrary commands if the untrusted strings aren't filtered to remove dangerous
 characters.  See function <variable>FILTER()</variable>.</para></warning>
                        </parameter>
                </syntax>
                <description>
                        <para>This action will fork audio from an ongoing call
 to the designated websocket serrver.</para> <variablelist> <variable
 name="AUDIOFORK_WSSERVER"> <para>The websocket server URL.</para>
                                </variable>
                        </variablelist>
                </description>
        </manager>
        <manager name="StopAudioFork" language="en_US">
                ...
                <syntax>
                        <xi:include
 xpointer="xpointer(/docs/manager[@name='Login']/syntax/parameter[@name='ActionID'])"
 /> <parameter name="Channel" required="true"> <para>The name of the channel
 monitored.</para>
                        </parameter>
                        <parameter name="AudioForkID" required="false">
                                <para>If a valid ID is provided, then this
 command will stop only that specific AudioFork.</para>
                ...
                <description>
                        <para>This command stops the audio fork that was created
 by the <literal>AudioFork</literal> action.</para>
                </description>
        </manager>
        <function name="AUDIOFORK" language="en_US">
                <synopsis>
                        Retrieve data pertaining to specific instances of
 AudioFork on a channel.
                </synopsis>
                <syntax>
                        <parameter name="id" required="true">
                                <para>The unique ID of the AudioFork instance.
 The unique ID can be retrieved through the channel variable used as an argument
 to the <replaceable>i</replaceable> option to AudioFork.</para>
                        </parameter>
                        <parameter name="key" required="true">
                                <para>The piece of data to retrieve from the
 AudioFork.</para> <enumlist> <enum name="filename" />
                                </enumlist>
                        </parameter>
                </syntax>
        </function>
...
static int audiofork_ws_close(struct audiofork *audiofork) {
  ...
  ast_verb(2,
           "[AudioFork] No reference to websocket, can't close connection\n");
  return -1;
}
...
audiofork_ws_connect(struct audiofork *audiofork) {
  enum ast_websocket_result result;

  if (audiofork->websocket) {
    ast_verb(2,
             "<%s> [AudioFork] (%s) Reconnecting to websocket server at: %s\n",
             ast_channel_name(audiofork->autochan->chan),
             audiofork->direction_string, audiofork->audiofork_ds->wsserver);
  ...
  if (audiofork->has_tls == 1) {
    ast_verb(2,
             "<%s> [AudioFork] (%s) Creating to WebSocket server with TLS mode "
             "enabled\n",
             ast_channel_name(audiofork->autochan->chan),
             audiofork->direction_string);
    audiofork->websocket = ast_websocket_client_create(
        audiofork->audiofork_ds->wsserver, "echo", audiofork->tls_cfg, &result);
  } else {
    ast_verb(2,
             "<%s> [AudioFork] (%s) Creating to WebSocket server without TLS\n",
             ast_channel_name(audiofork->autochan->chan),
             audiofork->direction_string);
    audiofork->websocket = ast_websocket_client_create(
...
static int audiofork_start_reconnecting(struct audiofork *audiofork) {
  ...
  while (counter < attempts) {
    ...
    ast_log(
        LOG_ERROR,
        "<%s> [AudioFork] (%s) Reconnection failed... trying again in %d "
        "seconds. %d attempts remaining reconn_now %d reconn_last_attempt %d\n",
        ast_channel_name(audiofork->autochan->chan),
        audiofork->direction_string, timeout, (attempts - counter), now,
        last_attempt);
...
static void *audiofork_thread(void *obj) {
  ...
  if (result != WS_OK) {
    ...
    /* We specifically don't do audiofork_free(audiofork) here because the
  ...
  ast_verb(2, "<%s> [AudioFork] (%s) Begin AudioFork Recording %s\n",
           ast_channel_name(audiofork->autochan->chan),
           audiofork->direction_string, audiofork->name);
  ...
  my_buff = ast_calloc(1, AF_MAX_BUFFER_SIZE);
  ...
  while (audiofork->audiohook.status == AST_AUDIOHOOK_STATUS_RUNNING) {
    ...
    for (cur = fr; cur; cur = AST_LIST_NEXT(cur, frame_list)) {
      ...
      if (current_buff_size + cur->datalen <= AF_MAX_BUFFER_SIZE) {
        memcpy(my_buff + current_buff_size, cur->data.ptr, cur->datalen);
        current_buff_size += cur->datalen;
      } else if (ast_websocket_write(audiofork->websocket,
                                     AST_WEBSOCKET_OPCODE_BINARY, my_buff,
                                     current_buff_size)) {

        ast_log(LOG_ERROR,
                "<%s> [AudioFork] (%s) Could not write to websocket.  "
                "Reconnecting...\n",
                ast_channel_name(audiofork->autochan->chan),
                audiofork->direction_string);
        ...
        if (ast_websocket_write(audiofork->websocket,
                                AST_WEBSOCKET_OPCODE_BINARY, my_buff,
                                current_buff_size)) {
          ast_log(LOG_ERROR,
                  "<%s> [AudioFork] (%s) Could not re-write to websocket.  "
                  "Complete Failure.\n",
                  ast_channel_name(audiofork->autochan->chan),
                  audiofork->direction_string);
          ...
          audiofork->audiohook.status = AST_AUDIOHOOK_STATUS_SHUTDOWN;
          current_buff_size = 0;
          memcpy(my_buff, cur->data.ptr, cur->datalen);
          current_buff_size += cur->datalen;
          break;
        }
        current_buff_size = 0;
    ...
    if (current_buff_size > 0) {
      ast_websocket_write(audiofork->websocket, AST_WEBSOCKET_OPCODE_BINARY,
                          my_buff, current_buff_size);
    }
  ...
  channel_name_cleanup =
      ast_strdupa(ast_channel_name(audiofork->autochan->chan));
  ...
  ast_verb(
      2,
      "<%s> [AudioFork] (%s) Finished processing audiohook. Frames sent = %d\n",
  ...
  if (audiofork->post_process) {
    ast_verb(2, "<%s> [AudioFork] (%s) Executing [%s]\n", channel_name_cleanup,
             audiofork->direction_string, audiofork->post_process);
...
static int launch_audiofork_thread(
  ...
  if (ast_audiohook_init(&audiofork->audiohook, AST_AUDIOHOOK_TYPE_SPY,
                         audiofork_spy_type, 0)) {
    audiofork_free(audiofork);
    return -1;
  }
  ...
  if (start_audiofork(chan, &audiofork->audiohook)) {
    ast_log(LOG_WARNING, "<%s> (%s) [AudioFork] Unable to add spy type '%s'\n",
            audiofork->direction_string, ast_channel_name(chan),
            audiofork_spy_type);
    ast_audiohook_destroy(&audiofork->audiohook);
    audiofork_free(audiofork);
    return -1;
...
static int audiofork_exec(struct ast_channel *chan, const char *data) {
  ...
  if (launch_audiofork_thread(chan, args.wsserver, flags.flags, direction,
...
static char *handle_cli_audiofork(struct ast_cli_entry *e, int cmd,
  ...
  case CLI_INIT:
    ...
    e->usage = "Usage: audiofork start <chan_name> [args]\n"
               "         The optional arguments are passed to the AudioFork "
               "application.\n"
               "       audiofork stop <chan_name> [args]\n"
               "         The optional arguments are passed to the "
               "StopAudioFork application.\n"
...
static int set_audiofork_methods(void) { return 0; }

static int clear_audiofork_methods(void) { return 0; }
...
static int load_module(void) {
  ...
  res |= ast_manager_register_xml("AudioForkMute",
                                  EVENT_FLAG_SYSTEM | EVENT_FLAG_CALL,
                                  manager_mute_audiofork);
  res |= ast_manager_register_xml("AudioFork", EVENT_FLAG_SYSTEM,
                                  manager_audiofork);
  res |= ast_manager_register_xml("StopAudioFork",
                                  EVENT_FLAG_SYSTEM | EVENT_FLAG_CALL,
